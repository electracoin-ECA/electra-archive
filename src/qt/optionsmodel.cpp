...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ECA);
    ...
    if (!settings.contains("nAnonymizeElectraAmount"))
        settings.setValue("nAnonymizeElectraAmount", 1000);
...
// Wallet
    ...
    if (settings.contains("nAnonymizeElectraAmount"))
        SoftSetArg("-anonymizeelectraamount", settings.value("nAnonymizeElectraAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElectraAmount:
            return QVariant(nAnonymizeElectraAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeElectraAmount:
            nAnonymizeElectraAmount = value.toInt();
            settings.setValue("nAnonymizeElectraAmount", nAnonymizeElectraAmount);
            emit anonymizeElectraAmountChanged(nAnonymizeElectraAmount);
            break;
        case CoinControlFeatures:
