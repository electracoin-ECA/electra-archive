...
{
    QString ret = QString("Electra:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ECA unit input in lnReqAmount, we generate the URI with ECA as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ElectraUnits::format(ElectraUnits::ECA, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
